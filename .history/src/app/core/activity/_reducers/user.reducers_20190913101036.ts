...
// Actions
import { UserActions, UserActionTypes } from '../_actions/activity.actions';
// CRUD
...
export interface UsersState extends EntityState<Activity> {
    listLoading: boolean;
...
export function usersReducer(state = initialUsersState, action: UserActions): UsersState {
    switch  (action.type) {
        ...
        case UserActionTypes.UserCreated: return adapter.addOne(action.payload.activity, {
            ...state, lastCreatedUserId: action.payload.activity.id
        });
...
export const getUserState = createFeatureSelector<UsersState>('activities');
