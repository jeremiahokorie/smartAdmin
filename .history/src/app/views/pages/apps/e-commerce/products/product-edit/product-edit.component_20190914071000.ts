...
export class ProductEditComponent implements OnInit, OnDestroy {
	...
	initProduct() {
		...
		if (!this.product.id) {
			this.subheaderService.setBreadcrumbs([
				...
				{ title: 'Create Layout', page: `../${prefix}/ecommerce/products/add` }
			]);
			return;
		}
		this.subheaderService.setTitle('Edit Layout');
		this.subheaderService.setBreadcrumbs([
			{ title: 'eCommerce', page: `../${prefix}/ecommerce` },
			{ title: 'Products',  page: `../${prefix}/ecommerce/products` },
			{ title: 'Edit Layout', page: `../${prefix}/ecommerce/products/edit`, queryParams: { id: this.product.id } }
		]);
	}
		...
		this.componentSubscriptions = this.store.pipe(
			...
			if (withBack) {
				this.goBack(newId);
			 	...
			 	const message = `New Layout successfully has been added.`;
	...
	updateProduct(_product: ProductModel, withBack: boolean = false) {
		...
		of(undefined).pipe(delay(3000)).subscribe(() => { // Remove this line
			if (withBack) {
				...
				const message = `Layout successfully has been saved.`;
	...
	getComponentTitle() {
		...
		result = `Edit Layout - ${this.product.manufacture} ${this.product.model}, ${this.product.modelYear}`;
		return result;
	}
