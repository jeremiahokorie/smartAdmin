...
export class UserEditComponent implements OnInit, OnDestroy {
	...
	initUser() {
		...
		if (!this.user.id) {
			...
			this.subheaderService.setBreadcrumbs([
				{ title: 'Activities', page: `activity-page` },
				{ title: 'Activities',  page: `activity-page` },
				{ title: 'Create Activity', page: `activity-page` }
			]);
			return;
		}
		this.subheaderService.setTitle('Edit Activity');
		this.subheaderService.setBreadcrumbs([
			{ title: 'Activity ', page: `activity-page` },
			{ title: 'Activities',  page: `activity-page` },
			{ title: 'Edit Activity', page: `activity-page/edit`, queryParams: { id: this.user.id } }
		]);
	}
	...
	refreshUser(isNew: boolean = false, id = 0) {
		...
		url = `${this.layoutConfigService.getCurrentMainRoute()}/activity/edit/${id}`;
		this.router.navigateByUrl(url, { relativeTo: this.activatedRoute });
	}
